...
# MAGIC %md
# MAGIC ## 0. Create a Delta Lake by calling the [Clinical Delta Lake](https://databricks.com/notebooks/00-etl-rwd.html) notebook 
# MAGIC 
...
# MAGIC %md
# MAGIC Next, we define a function that for every comborbid condition adds a binary feature indicating whether a given encounter was related to the that condition.
...
# MAGIC %md
# MAGIC Finally, we to construct comborbidity features that capture the number of times a patient has been diagnosed with any of the comorbid conditions within a given window of time.
...
def add_recent_encounters(encounter_features):
  ...
  for comorbidity_idx in range(num_conditions):
      col_name = f"recent_{comorbidity_idx}"
      
      output_df = (
        output_df
        .withColumn(col_name, sum(col(f"comorbidity_{comorbidity_idx}")).over(w))
...
# MAGIC %md
# MAGIC Now let's take a look at the distribution of labels 
...
def pre_process(training_dataset_pdf):
  ...
  one_hot_model = onehotencoder.fit(X_pdf)
...
comorbidty_cols = [f'recent_{idx}' for idx in range(0,num_conditions)] + [f'comorbidity_{idx}' for idx in range(0,num_conditions)]
...
spark_trials = SparkTrials(parallelism=sc.defaultParallelism)
best_params = fmin(
  fn=tune_model,
  space=search_space,
  algo=tpe.suggest,
...
def train(params):
  with mlflow.start_run(run_name='training-logistic-regression',nested=True) as run:
    ...
    X_arr=training_dataset_pdf.drop('label',axis=1)
...
# MAGIC %md
# MAGIC 
# MAGIC Now you can register the trained model in model registry (you can also use GUI for this)
...
clf_udf = mlflow.pyfunc.spark_udf(spark, model_uri)

# COMMAND ----------

features_df = spark.read.load(delta_root_path+'/encounter_features').limit(100)

# COMMAND ----------

(
  features_df
  .select('PATIENT', 'Enc_Id', 'START_TIME', clf_udf(*selected_features).alias('risk_score'))
).display()
